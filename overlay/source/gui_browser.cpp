...
BrowserGui::BrowserGui()
    ...
    this->m_list->addItem(new tsl::elm::CategoryHeader("Не удалось открыть SD-карту"));
    }
}
...
tsl::elm::Element *BrowserGui::createUI() {
    ...
    m_frame->setDescription("\uE0E1  Назад     \uE0E0  Добавить    \uE0E2  Добавить всё");
...
void BrowserGui::scanCwd() {
    ...
    if (R_FAILED(rc)) {
        ...
        this->m_list->addItem(new tsl::elm::ListItem("Что-то пошло не так :/"));
        ...
        if (entry.type == FsDirEntryType_Dir) {
            ...
            item->setClickListener([this, item](u64 down) -> bool {
                if (down & HidNpadButton_A) {
                    ...
                    if (R_SUCCEEDED(rc)) {
                        m_frame->setToast("Плейлист обновлён", "Добавлена 1 композиция в плейлист.");
                    } else {
                        m_frame->setToast("Не удалось добавить трек.", "Имя файла содержит недопустимые символы?");
                    }
                    return true;
                }
                return false;
    ...
    if (folders.size() == 0 && files.size() == 0) {
        this->m_list->addItem(new tsl::elm::CategoryHeader("Пусто..."));
        return;
    }
    ...
    if (files.size() > 0) {
        this->m_list->addItem(new tsl::elm::CategoryHeader("Файлы"));
...
void BrowserGui::addAllToPlaylist() {
    ...
    std::snprintf(path_buffer, sizeof(path_buffer), "Добавлено %ld композиций в плейлист.", songs_added);
    m_frame->setToast("Плейлист обновлён", path_buffer);
}
