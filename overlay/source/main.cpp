  ...
  public:
    void initServices() override {
        if (R_FAILED(pm::Initialize())) {
            this->msg  = "Не удалось pm::Initialize()";
            return;
        }

        // don't open RyazhTune if blacklisted title is active!
        u64 pid{}, tid{};
        pm::getCurrentPidTid(&pid, &tid);

        if (config::get_title_blacklist(tid)) {
            this->msg =
                "Тайтл в чёрном списке!\n"
                "Выйдите, чтобы использовать RyazhTune";
            return;
        }
        ...
        if (R_FAILED(sdmc::Open())) {
            this->msg  = "Не удалось sdmc::Open()";
            return;
        }

        u32 api;
        if (R_FAILED(tuneGetApiVersion(&api)) || api != TUNE_API_VERSION) {
            this->msg = "   Неподдерживаемая\n"
                        "версия RyazhTune!";
        }
    }
