...
- **5-полосный эквалайзер**: Собственная реализация с единым live-тюнером из пяти вертикальных фейдеров (100 Гц, 300 Гц, 1 кГц, 3 кГц и 10 кГц), мгновенным применением и готовыми профилями. На фейдере `A` включает редактирование, `L/R` выбирает полосу, `↑/↓` меняет усиление, а `A` или `B` возвращает управление списку; поддерживается и вертикальная сенсорная регулировка. Переключатель назначения выбирает музыку RyazhTune (программный DSP) либо игру/систему (аппаратный EQ аналогового выхода ALC5639; HDMI в доке не поддерживается).
...
- `config.ini`: Основные параметры конфигурации (автозапуск, громкость, перемешивание, повтор, язык, фильтрация по Title ID и секция `equalizer`).
...
- **Текущая версия:** <!-- CURRENT_VERSION_START -->5.6.0<!-- CURRENT_VERSION_END -->
- **Статус:** Стабильный
