            ...
            /* ---- (1) Tid-change edge: update current_tid + volume ---- */
            {
                ...
                if (pm::PollCurrentPidTid(&new_pid, &new_tid)) {
                    ...
                    if (first_poll) {
                        ...
                        policyWrite(!resolveTitleEnabled(new_tid));
                        current_tid = new_tid;
                        first_poll  = false;
                    } else {
                        /* Genuine title change (different process foreground).
                         * Apply the new title's Pause On Start policy. */
                        policyWrite(!resolveTitleEnabled(new_tid));
                        current_tid  = new_tid;
                ...
                if (R_SUCCEEDED(rc)) {
                    ...
                    if (new_focused != last_focused) {
                        if (!new_focused) {
                             ...
                             * policy. If the user subsequently Pauses or
                             * Plays during the HOME visit, Pause()/Play()
                             * will keep g_saved_pause_state in sync so
                             * the eventual focus-regain restores their
                             * latest intent rather than a stale value. */
                            g_saved_pause_state = g_should_pause;
                            policyWrite(!resolveTitleEnabled(kHomeScreenTid));
                        } else {
