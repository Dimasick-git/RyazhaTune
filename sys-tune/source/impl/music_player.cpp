...
namespace tune::impl {
    ...
    namespace {
        ...
        bool endsWithInsensitive(const char *name, const char *ext) {
            size_t ln = std::strlen(name), le = std::strlen(ext);
            if (ln < le) return false;
            const char *start = name + (ln - le);
            for (size_t i = 0; i < le; ++i) {
                unsigned char a = static_cast<unsigned char>(start[i]);
        ...
        bool supportsAudio(const char *name) {
            bool ok = false;
#ifdef WANT_MP3
            ok = ok || endsWithInsensitive(name, ".mp3");
#endif
#ifdef WANT_FLAC
            ok = ok || endsWithInsensitive(name, ".flac");
#endif
#ifdef WANT_WAV
            ok = ok || endsWithInsensitive(name, ".wav") || endsWithInsensitive(name, ".wave");
#endif
            return ok;
        }

        void scanAndPopulateDefaultPlaylist(size_t maxFiles) {
            if (maxFiles == 0) return;

            FsFileSystem fs;
            if (R_FAILED(fsOpenSdCardFileSystem(&fs))) return;

            auto joinPath = [](char *dst, size_t dstSize, const char *dir, const char *leaf, bool addSlash) {
                if (dstSize == 0) return;
                size_t written = 0;
                dst[0] = '\0';

                auto append = [&](const char *s) {
                    size_t rem = (written < dstSize) ? (dstSize - 1 - written) : 0;
                    if (rem == 0) return;
                    size_t l = std::strlen(s);
                    if (l > rem) l = rem;
                    memcpy(dst + written, s, l);
                    written += l;
                    dst[written] = '\0';
                };
                ...
                append(dir);
                append(leaf);
                if (addSlash && written + 1 < dstSize) {
                    dst[written++] = '/';
                    dst[written] = '\0';
                }
            };

            std::function<void(const char*, size_t&)> walk;
            walk = [&](const char* dirPath, size_t &added) {
                ...
                while (R_SUCCEEDED(fsDirRead(&dir, &count, 1, &entry)) && count) {
                    if (entry.type == FsDirEntryType_Dir) {
                        ...
                        joinPath(child, sizeof(child), dirPath, entry.name, true);
                        walk(child, added);
                        if (added >= maxFiles) break;
                    } else if (supportsAudio(entry.name)) {
                        char full[FS_MAX_PATH];
                        joinPath(full, sizeof(full), dirPath, entry.name, false);
                        {
            ...
            size_t added = 0;
            FsDir test;
            if (R_SUCCEEDED(fsFsOpenDirectory(&fs, "/music/", FsDirOpenMode_ReadDirs, &test))) {
                fsDirClose(&test);
                walk("/music/", added);
            } else {
                walk("/", added);
            ...
            if (added > 0) {
                ...
                for (const auto& entry : *g_playlist) paths.push_back(entry.path);
    ...
    Result Initialize(std::vector<PlaylistEntry>* playlist, std::vector<PlaylistID>* shuffle, PlaylistEntry* current) {
        ...
        // Initialize paused/playing: if autoplay is enabled, start playing;
        // otherwise start paused.
        bool autoplay = config::get_autoplay_enabled();
        g_should_pause = !autoplay;

        // If autoplay is enabled but playlist is empty, try auto-scan to fill it
        if (autoplay && g_playlist->empty()) {
            scanAndPopulateDefaultPlaylist(200);
        }

        // If autoplay is enabled and we have tracks, force a start
        if (autoplay && !g_playlist->empty()) {
    ...
    void Exit() {
        ...
        // Persist current playing state on exit
        config::set_last_playing_state(!g_should_pause);
    }
    ...
    void PmdmntThreadFunc(void *ptr) {
        ...
        while (g_should_run) {
            ...
            if (pm::PollCurrentPidTid(&pid, &new_tid)) {
                // Title switched; reset handling below will decide pause state
                    ...
                    if (whitelist_mode) {
                        ...
                        if (autoplay_enabled) {
                            if (config::has_title_enabled(new_tid)) {
                                desired_pause = !config::get_title_enabled(new_tid);
                            } else {
                                desired_pause = !config::get_title_enabled_default();
                            }
                        }
                    }
                    // Apply immediately (удалён grace-период)
                    g_should_pause = desired_pause;
    ...
    void Play() {
        g_should_pause = false;
        config::set_last_playing_state(true);
    }

    void Pause() {
        g_should_pause = true;
        config::set_last_playing_state(false);
    }
